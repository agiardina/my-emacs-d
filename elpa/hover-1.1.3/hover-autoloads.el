...
Start `hover run` or hot-reload if already running." t nil)

(autoload 'hover-run-or-hot-restart "hover" "\
Start `hover run` or hot-restart if already running." t nil)

(autoload 'hover-take-screenshot "hover" "\
Take screenshot of current `hover` application using `flutter screenshot`.
Saves screenshot on `hover-screenshot-path`." t nil)
